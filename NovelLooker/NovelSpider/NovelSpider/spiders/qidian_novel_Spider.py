...
import scrapy
import sys
sys.path.append("D:\\GitSpace\\NovelLooker")
...
class QidianNovelSpider(scrapy.Spider):
    ...
    def parse_novel_item(self,res):
        ...
        novel_tag_2=tags_2=novel_info.xpath('.//p[contains(@class,"tag")]/a/text()')
        ...
        novel_chapters=res._url+'#Catalog'

        item['novelname']=novel_name
        item['novelauthor']=novel_author
        item['novelcover']=novel_img_url
        item['novelintro']=novel_intro
        item['noveltags']=novel_tags
        item['novelchapters']=novel_chapters
        item['novelcrawltime']=datetime.datetime.now()
        #爬取novel_item说需要的所有的属性，然后返回
